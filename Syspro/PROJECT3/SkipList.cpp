...
void SLL::vaccineStatus(string ID, string vName){
    SLL* temp = head;
    while(temp!=NULL){
        if (temp->sl->getVirusName() == vName)
        {
            ...
            if(c!=NULL){
                if(c->getVaccinated() == "YES")
                    cout << "VACCINATED ON " << c->getDateVaccinated() << endl;
                else
...
void SLL::findAllRecords(string ID){
    SLL* temp = head;
    bool printCitizen = false;
    while(temp!=NULL){
        citizen *c = temp->sl->check(atoi(&ID[0]));
        if(c!=NULL){

            if(printCitizen == false){
                printCitizen = true;
                cout << c->getCitizenID() << " " << c->getFirstName() << " " << c->getLastName() << " " << c->getCountry() << endl;
                cout << "AGE " << c->getAge() << endl;
            }
            
            if(c->getVaccinated() == "YES")
                cout << temp->sl->getVirusName() << " VACCINATED ON " << c->getDateVaccinated() << endl;
            else
                cout << temp->sl->getVirusName() << " NOT YET VACCINATED" << endl;
        }
        temp = temp->next;
    }
    return;
